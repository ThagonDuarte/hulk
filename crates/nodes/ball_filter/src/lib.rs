...
mod tests {
    ...
    fn test_parameters() -> BallFilterParameters {
        ...
        parameters.hypothesis_timeout = Duration::from_secs(10);
        parameters.maximum_number_of_hypotheses = 10;
        ...
        parameters.noise.process_noise_moving = nalgebra::vector![0.0, 0.0, 0.0, 0.0];
        parameters.noise.process_noise_resting = nalgebra::vector![0.0, 0.0];
        parameters.noise.initial_covariance = nalgebra::vector![1.0, 1.0, 1.0, 1.0];
        parameters
    }

    fn resting_filter_at(x: f32, y: f32) -> BallFilter {
        BallFilter {
            hypotheses: vec![BallHypothesis {
                ...
                last_seen: Time::from_nanos(1_000_000_000),
                validity: 1.0,
            }],
        }
    }
    ...
    #[test]
    fn odometry_entry_predicts_hypotheses_without_detection() {
        const EPSILON: f32 = 1e-6;

        let mut ball_filter = resting_filter_at(1.0, 0.0);
        let mut last_odometer = None;
        let mut last_prediction_time = None;
        ...
        let position = ball_filter.hypotheses[0].position().position;
        assert!(position.x().abs() <= EPSILON);
        assert!((position.y() + 1.0).abs() <= EPSILON);
    }

    #[test]
    fn detection_entry_advances_without_odometry_prediction_arguments() {
        const EPSILON: f32 = 1e-6;
        ...
        assert_eq!(ball_filter.hypotheses.len(), 1);

        let position = ball_filter.hypotheses[0].position().position;
        assert!((position.x() - 1.0).abs() <= EPSILON);
        assert!(position.y().abs() <= EPSILON);
    }

    #[test]
    fn cleanup_runs_independently_after_persistent_batch() {
        let mut ball_filter = resting_filter_at(10.0, 0.0);
        let parameters = test_parameters();
        ...
        assert!(ball_filter.hypotheses.is_empty());
    }

    #[test]
    fn detection_advancement_discards_below_threshold_before_matching() {
        const EPSILON: f32 = 1e-6;

        let mut ball_filter = resting_filter_at(1.0, 0.0);
        ball_filter.hypotheses[0].validity = 0.1;

        let mut parameters = test_parameters();
        parameters.validity_discard_threshold = 0.5;
        ...
        assert_eq!(ball_filter.hypotheses.len(), 1);
        assert!((ball_filter.hypotheses[0].validity - 1.0).abs() <= EPSILON);
    }
