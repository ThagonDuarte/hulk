...
async fn run(ctx: Arc<Context>) -> Result<()> {
    ...
    let non_maximum_suppression_duration_pub = node
        ...
        .build()
        .await?;
    let detected_yolo_objects_pub = node
        .announcing_publisher::<Vec<Object<YOLOObjectLabel>>>("detected_yolo_objects")
        .await?;
    let detected_custom_objects_pub = node
        .announcing_publisher::<Vec<Object<CustomObjectLabel>>>("detected_objects")
        .await?;
    let detected_poses_pub = node
        .announcing_publisher::<Vec<FieldFeaturePose>>("detected_poses")
        .await?;
    ...
    loop {
        ...
        let detected_yolo_objects_pending = detected_yolo_objects_pub.announce(image_time).await?;
        let detected_custom_objects_pending =
            detected_custom_objects_pub.announce(image_time).await?;
        ...
        let detected_custom_objects = non_maximum_suppression(
        ...
        detected_yolo_objects_pending
            .publish(&detected_yolo_objects)
            .await?;
        detected_custom_objects_pending
            .publish(&detected_custom_objects)
            .await?;
