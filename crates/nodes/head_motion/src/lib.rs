...
async fn run(ctx: Arc<Context>) -> Result<()> {
    ...
    let mut latest_look_around_target_joints = None;
    let mut latest_look_at = None;
    let mut latest_motor_states = None;
    ...
    loop {
        tokio::select! {
            look_around_target_joints = look_around_target_joints_sub.recv() => {
                latest_look_around_target_joints = Some(look_around_target_joints?);
            }
            look_at = look_at_sub.recv() => {
                latest_look_at = Some(look_at?);
            }
            motor_states = motor_states_sub.recv() => {
            ...
            _ = tick.tick() => {
                for _ in 0..MAX_INPUT_DRAIN_PER_TICK {
                    if !look_around_target_joints_sub.is_ready() {
                        break;
                    }

                    latest_look_around_target_joints = Some(look_around_target_joints_sub.recv().await?);
                }

                for _ in 0..MAX_INPUT_DRAIN_PER_TICK {
                    if !look_at_sub.is_ready() {
                        break;
                    }

                    latest_look_at = Some(look_at_sub.recv().await?);
                }

                for _ in 0..MAX_INPUT_DRAIN_PER_TICK {
                    if !motor_states_sub.is_ready() {
                        break;
                    }

                    latest_motor_states = Some(motor_states_sub.recv().await?);
                }
                ...
                let Some(look_at) = latest_look_at else {
                    continue;
                };
                let Some(motor_states) = latest_motor_states.as_ref() else {
                    continue;
                };
...
fn cycle_duration_since_last_update(last_update: &mut Option<Instant>, now: Instant) -> Duration {
...
fn joints_from_motion(
    ...
    match motion_command.head_motion() {
        ...
        | Some(HeadMotion::LookLeftAndRightOf { .. }) => look_at,
