...
impl BallFilter {
    ...
    fn advance_all_hypotheses(
        ...
        for (detection_time, (last_to_current, balls)) in
            self.align_odometry_and_percepts(ball_percepts, odometries)
        {
    ...
    fn align_odometry_and_percepts(
        ...
        for (time, percepts) in ball_percepts {
            let delta = match self.last_odometer {
                None => {
                    self.last_odometer = odometries
                        .get(&time)
                ...
                Some(last_odometer) => {
                    let odometer_state = odometries
                        .get(&time)
                        .and_then(|v| v.last().copied())
                        .copied()
                        .unwrap_or(last_odometer);
                    let delta = last_odometer.to(odometer_state);
                    self.last_odometer = Some(odometer_state);
                    delta
                }
            };
            output.insert(time, (delta, percepts));
        }
    ...
    pub fn cycle(&mut self, mut context: CycleContext) -> Result<MainOutputs> {
        ...
        let projected_balls = projected_balls(
            context.detected_objects.persistent,
...
fn projected_balls(
    detections: BTreeMap<SystemTime, Vec<&Vec<Object<CustomObjectLabel>>>>,
