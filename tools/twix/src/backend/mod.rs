...
impl TwixBackend {
    ...
    pub fn new_with_keep_connected(
        ...
        let (router_endpoint_sender, router_endpoint_receiver) = watch::channel(router_endpoint);
    ...
    pub fn set_router_endpoint(&self, router_endpoint: impl Into<String>) -> Result<()> {
        ...
        let router_endpoint = self.router_endpoint();
        if self.keep_connected()
            && self.connection_state_sender.borrow().router_endpoint()
                != Some(router_endpoint.as_str())
        {
            self.connection_state_sender
                .send_replace(ConnectionState::connecting(router_endpoint));
        }
        Ok(())
    }
    ...
    pub fn set_keep_connected(&self, keep_connected: bool) {
        ...
        let state = if keep_connected {
            ConnectionState::connecting(self.router_endpoint())
        } else {
            ConnectionState::disconnected()
        };
        self.connection_state_sender.send_replace(state);
    }

    pub fn connection_status(&self) -> ConnectionStatus {
        self.connection_state_sender.borrow().status()
    }
