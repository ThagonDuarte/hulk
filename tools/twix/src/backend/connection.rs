...
impl ConnectionState {
    ...
    pub(crate) fn connecting(router_endpoint: String) -> Self {
        ...
        Self::Connecting {
            router_endpoint: string_to_arc_str(router_endpoint),
            message: string_to_arc_str(message),
        }
    }

    pub(crate) fn connected(router_endpoint: String, node: Arc<Node>) -> Self {
        Self::Connected {
            router_endpoint: string_to_arc_str(router_endpoint),
            node,
        }
    ...
    pub(crate) fn router_endpoint(&self) -> Option<&str> {
        match self {
    ...
    pub(crate) fn unavailable_message(&self) -> Option<&str> {
        match self {
            ...
            Self::Connecting { message, .. } => Some(message),
            Self::Connected { .. } => None,
...
async fn run_connection_task(
    ...
    loop {
        ...
        match result {
            Ok(node) => {
                connection_state.send_replace(ConnectionState::connected(endpoint, node));
                egui_context.request_repaint();
