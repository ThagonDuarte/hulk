...
impl Layer<Field> for BallPosition {
    ...
    fn new(backend: Arc<TwixBackend>) -> Self {
        let ground_to_field =
            backend.subscribe_buffered_value("ground_to_field", Duration::from_secs(2));
        let ball_position =
            backend.subscribe_buffered_value("ball_filter/ball_position", Duration::from_secs(2));
        let team_ball = backend.subscribe_value("team_ball");
        Self {
            ground_to_field,
            ball_position,
            team_ball,
    ...
    fn paint(
        ...
        for (ball, ground_to_field) in ball_positions.iter().zip(ground_to_fields.iter()) {
            ...
            let ground_to_field = ground_to_field.value;
        ...
        if let Some(ball) = self.ball_position.get_last_value()?.flatten() {
            let ground_to_field = self.ground_to_field.get_last_value()?.unwrap_or_default();
            painter.ball(
                ground_to_field * ball.position,
                field_dimensions.ball_radius,
                Color32::WHITE,
